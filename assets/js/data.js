...
const DataManager = {
  ...
  // ---- ADMIN AUTH (localStorage uniquement) ----
  isAdmin() { return localStorage.getItem('ptm_admin') === 'true'; },
  loginAdmin(pwd) {
    const ok = pwd === (localStorage.getItem('ptm_admin_pwd') || 'Minouches2024!');
    if (ok) localStorage.setItem('ptm_admin', 'true');
    return ok;
  },
  logoutAdmin() { localStorage.removeItem('ptm_admin'); },
